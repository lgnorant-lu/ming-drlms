...
def test_enqueue_and_process():
    ...
    # Enqueue a command
    cmd = UICommand(
        type=UICommandType.ADD_MESSAGE, data={"message": "test"}, source="test"
    )
    queue.enqueue(cmd)
...
def test_batch_processing():
    ...
    for i in range(5):
        cmd = UICommand(
            type=UICommandType.ADD_MESSAGE, data={"message": f"test{i}"}, source="test"
        )
        queue.enqueue(cmd)
...
def test_unknown_command_type():
    ...
    cmd = UICommand(type=UICommandType.UPDATE_USER_LIST, data={}, source="test")
    queue.enqueue(cmd)
...
def test_handler_error_handling():
    ...
    queue.enqueue(UICommand(type=UICommandType.ADD_MESSAGE, data={}, source="test"))
    queue.enqueue(
        UICommand(type=UICommandType.UPDATE_USER_LIST, data={}, source="test")
    )
...
def test_stats_tracking():
    ...
    for i in range(3):
        queue.enqueue(
            UICommand(type=UICommandType.ADD_MESSAGE, data={"i": i}, source="test")
        )
