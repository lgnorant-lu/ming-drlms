...
def test_demo_quickstart_skips_when_binaries_missing(monkeypatch):
    ...
    # Force BIN_AGENT and BIN_SERVER missing for all modules
    class _P:
    ...
    # Mock in all relevant modules
    monkeypatch.setattr(demo_mod, "BIN_AGENT", _P(False))
    monkeypatch.setattr(demo_mod, "BIN_SERVER", _P(False))
    monkeypatch.setattr(client_mod, "BIN_AGENT", _P(False))
    monkeypatch.setattr(utils, "BIN_AGENT", _P(False))
    monkeypatch.setattr(utils, "BIN_SERVER", _P(False))
