...
int main(void) {
    ...
    char big[1500]; memset(big, 'X', sizeof(big)); big[sizeof(big)-1] = '\n';
    ...
    ssize_t n1 = shm_read(out, sizeof(out)); assert(n1 > 0);
    out[n1 < (ssize_t)sizeof(out) ? n1 : (ssize_t)sizeof(out)-1] = 0;
    printf("%s", out);

    ssize_t n2 = shm_read(out, sizeof(out)); assert(n2 > 0);
    out[n2 < (ssize_t)sizeof(out) ? n2 : (ssize_t)sizeof(out)-1] = 0;
    printf("%s", out);

    ssize_t n3 = shm_read(out, sizeof(out)); assert(n3 == sizeof(big));
