...
// Initialize rooms subsystem. base_dir is the server data dir (e.g., "server_files").
// The implementation will ensure a subdirectory base_dir/rooms exists.
...
// Remove a subscriber fd from all rooms (used when a client disconnects unexpectedly).
...
// Store text event to disk (events log + payload file). Returns 0 and out_event_id on success.
