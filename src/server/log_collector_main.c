    ...
    if (ctx_rc != 0 || !ctx.instance) {
        ...
        if (fb_room) {
            ...
            if (!target_instance) {
                ...
                RoomAssignResult assign_rc =
                    rooms_assign_instance(fb_room, NULL, &ctx.instance_uuid,
                                          &target_instance, &is_new_instance);
                if (assign_rc != ROOM_ASSIGN_OK || !target_instance) {
                    platform_mutex_unlock(&fb_room->mu);
                    free(payload);
                    return legacy_sendf(session->fd,
                                        "ERR|PUBT|prepare failed\n");
                }
            }
...
static void *handle_client(void *arg) {
    ...
    if (!use_mp2) {
        ...
        setsockopt(fd, SOL_SOCKET, SO_RCVTIMEO, (const char *)&tv_reset,
                   sizeof(tv_reset));
#else
