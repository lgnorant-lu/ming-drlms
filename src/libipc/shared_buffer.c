...
static key_t derive_key(void) {
    ...
    if (!env || !*env) return (key_t)0x4c4f4742; // default 'LOGB'
    char* endptr = NULL;
    unsigned long val = strtoul(env, &endptr, 0); // auto-detect base (0x.. or decimal)
...
int shm_write(const unsigned char* data, size_t len) {
    if (!shared) { errno = EINVAL; return -1; }
    size_t offset = 0; uint32_t seq = 0;
    while (offset < len) {
        size_t payload = (len - offset);
        size_t max_payload = (MAX_MSG_SIZE > sizeof(MsgHdr)) ? (MAX_MSG_SIZE - sizeof(MsgHdr)) : 0;
        if (payload > max_payload) payload = max_payload;
        MsgHdr hdr; hdr.len = (uint32_t)payload; hdr.seq = seq++; hdr.flags = 0;
        if (offset + payload >= len) hdr.flags |= LAST_FLAG;

        sem_wait(&shared->sem_empty);
        pthread_rwlock_wrlock(&shared->rwlock);
        memcpy(shared->buffer[shared->write_index], &hdr, sizeof(MsgHdr));
        memcpy(shared->buffer[shared->write_index] + sizeof(MsgHdr), data + offset, payload);
...
ssize_t shm_read(unsigned char* out, size_t out_size) {
    ...
    for (;;) {
        ...
        const unsigned char* src = shared->buffer[shared->read_index] + sizeof(MsgHdr);
        size_t copy = (total < out_size) ? ((out_size - total) < payload ? (out_size - total) : payload) : 0;
        if (copy > 0) memcpy(out + total, src, copy);
        total += payload;
        shared->read_index = (shared->read_index + 1) % NUM_SLOTS;
        shared->count--;
        pthread_rwlock_unlock(&shared->rwlock);
        sem_post(&shared->sem_empty);
        if (hdr.flags & LAST_FLAG) break;
    }
